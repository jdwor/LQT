...
get_patient_sspl<-function(cfg){
  ...
  if(!dir.exists(pat.path)){
    stop('Patient folder does not exist in output directory; cannot compute changes in SSPLs until patient SSPL matrix has been created. Please create patient SSPL matrix and retry.');
  }
